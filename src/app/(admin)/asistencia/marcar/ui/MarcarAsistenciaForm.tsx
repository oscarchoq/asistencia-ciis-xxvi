...
import { marcarAsistencia } from "@/actions/asistencia/marcar";
...
export default function MarcarAsistenciaForm() {
  ...
  // Función para marcar asistencia (usada por ambos métodos)
  const handleMarcarAsistencia = async (numeroDocumento: string) => {
    ...
    try {
      const result = await marcarAsistencia(numeroDocumento);
      ...
      await Swal.fire({
        ...
        text: "Ocurrió un error al marcar la asistencia. Por favor, intente nuevamente.",
        confirmButtonText: "Entendido",
        confirmButtonColor: "#ef4444",
      });
      console.error(error);
  ...
  // Callback cuando se escanea un QR - marca automáticamente
  const handleQRScanned = async (code: string) => {
    console.log("QR Escaneado:", code); // Debug
    setShowScanner(false);
    await handleMarcarAsistencia(code);
  };
  ...
  const handleManualSubmit = async () => {
    ...
    await handleMarcarAsistencia(codigoEncriptado);
  };
          ...
          <div className="flex flex-col items-center gap-4 rounded-lg border-2 border-dashed border-[#dadbdd] p-8 text-center dark:border-neutral-700">
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <p className="max-w-xs text-sm font-normal leading-normal text-[#6e7277] dark:text-neutral-400">
                Coloque el código QR frente a la cámara para registrar su asistencia automáticamente.
              </p>
            </div>
